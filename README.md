...
- **?? Live Production Web App:** [https://frontend-alpha-five-43.vercel.app/](https://frontend-alpha-five-43.vercel.app/)
...
- **?? Backend Health Check:** [https://groww-since-you-checked-watchlist.onrender.com/health](https://groww-since-you-checked-watchlist.onrender.com/health)
...
| :--- | :--- |
| **1. 4-Digit Security & Auth Modal** | <img src="screenshots/01_auth_modal_pincode.png" width="460" alt="Auth Modal" /><br>**Detailed Explanation:**<br>? Clean, tactile login and sign-up modal enforcing a strict 4-digit numeric PIN constraint.<br>? Synchronizes user identity cross-device, linking portfolios, custom watchlists, and session audit logs directly to Supabase cloud storage. |
| **2. Dynamic "Since You Checked" Intelligence Header** | <img src="screenshots/02_dynamic_since_you_checked_header.png" width="460" alt="Dynamic Intelligence Header" /><br>**Detailed Explanation:**<br>? Displays dynamic time delta since user's previous session (e.g. *?Just now?*, *?15m ago?*, *?2h ago?*).<br>? Real-time stat metrics show Top Gainers, Top Losers, Volume Surges, and NIFTY 50 market trend.<br>? Includes **Voice Audio Briefing (?? Listen Briefing)**, **Voice Search (???)**, and live portfolio balance badge (?1,22,512.80). |
| **3. Master 202-Stock Split Dashboard & Trajectory** | <img src="screenshots/03_dashboard_split_watchlist_trajectory.png" width="460" alt="Split Dashboard" /><br>**Detailed Explanation:**<br>? Left panel features a searchable 202+ NSE Master Directory with sector pills (Tech, Banking, Auto, Energy, Defense, etc.).<br>? Right detail panel displays live candlestick trajectory sparklines, 20-day moving averages, 52-week High/Low ranges, and high-contrast **Red Sell / Green Buy** action buttons. |
...
## ?? How to Test Edge Cases (Penny Stocks & Broken Stocks)
...
- **How TrackPulse Handles It:**
  1. Identifies that the stock fails the ?2 Crore turnover threshold.
  2. Sets confidence: "low" and is_illiquid: true.
  3. Displays an explicit warning: ?? Low liquidity warning ? trading volume or volatility too low for reliable anomaly scoring.
- **How to Test in UI & Backend:**
  - **In Web App:** Click **+ Add Stock**, search for **PENNYTEST**, and add it to your watchlist. Inspect the stock card to see the amber low-liquidity warning badge and low-confidence indicator.
  - **In Automated CLI:** Run python backend/test_engine.py (executes 	est_06_low_liquidity_detection_pennytest).

---

### 2. ?? Broken Stock / Exchange Outage / Network Dropouts
...
- **How TrackPulse Handles It:**
  1. Wraps all external calls in isolated try/catch safety boundaries with structured error models:
     ...
     `
  2. Renders an isolated fallback warning card without breaking any neighboring stock calculations or blocking the session checkpoint.
- **How to Test in UI & Backend:**
  - **In Web App:** Click **+ Add Stock**, add **BROKENSTOCK** or any unquoted symbol. Notice that the rest of the watchlist operates smoothly while BROKENSTOCK displays an isolated offline warning.
  - **In Automated CLI:** Run python backend/test_engine.py (executes 	est_07_graceful_error_handling_brokenstock).

---

## ?? Quantitative Anomaly Scoring Formula

Instead of raw sorting, conviction score ( - 100$) is computed via a 3-factor volatility-normalized model:
...
1. **Price $-Score Normalization (40% Weight):** Measures price delta relative to historical standard deviation ($\sigma$).
2. **Volume Breakout Multiplier (35% Weight):** Flags unusual institutional activity exceeding .5\times - 2.5\times$ baseline volume.
3. **NIFTY 50 Decoupled Alpha (25% Weight):** Identifies independent relative strength ({\text{stock}} - R_{\text{NIFTY}}$).
...
## ??? Alignment with Groww's 5 Core Values
...
3. **Being Transparent:** Auditable mathematical formula attribution (40% P ? 35% V ? 25% ?) and clear checkpoint metrics.
