...
| **8. Fault Isolation & Exchange Outage Resilience (`BROKEN` Stock)** | <img src="screenshots/08_fault_isolation_broken_stock_resilience.png" width="460" alt="Exchange Outage Fault Isolation" /><br>**Detailed Explanation:**<br>• Demonstrates granular React Error Boundaries & high-availability architecture during simulated exchange API timeouts (HTTP 504).<br>• Displays an isolated rose warning banner: *"Exchange Connection Timeout (Simulated Outage)"* and a dedicated *"Live Trajectory Stream Disconnected"* container.<br>• Automatically sets Support Floor, Target Price, and Stop Loss to `N/A`, while safely disabling Buy & Sell buttons to prevent invalid orders.<br>• **Zero-Crash Resilience:** All neighboring 202+ watchlist stocks (`ZOMATO`, `HAL`, `IDEA`, `TATAMOTORS`) continue uninterrupted live streaming. |
